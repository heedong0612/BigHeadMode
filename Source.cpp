...
Mat fetchFace(const Mat& input, int width, int height, Point center, double angle = 0) {
...
int main(int argc, const char** argv) {
	...
	Mat original2 = imread(filename2);

	// resize
	while (original.rows * original.cols >= 500000) //more than 500 x 500
		resize(original, original, Size(), 0.5, 0.5);

	while (original2.rows * original2.cols >= 500000) //more than 500 x 500
		resize(original2, original2, Size(), 0.5, 0.5);
	...
	//Approach 1:
	// Detects the faces using haar cascade and enlarges them. If the faces are tilted, it adjusts the anagles.
	imwrite("original.jpg", original);
	Mat enlarged = detectAndDisplay(original); 
	imwrite("originalAfterEnlarged.jpg", original);

	// Approach 2: 
	// Detects the face and extracts it using edge detection. After masking, it enlarges the face.
	imwrite("original2.jpg", original2);
	Mat enlarged2 = detectAndDisplayUsingEdgeDetection(original2);
	imwrite("originalAfterEnlarged2.jpg", original2);
